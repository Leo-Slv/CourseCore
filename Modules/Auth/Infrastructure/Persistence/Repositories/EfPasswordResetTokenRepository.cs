...
public class EfPasswordResetTokenRepository : IPasswordResetTokenRepository
{
    ...
    public EfPasswordResetTokenRepository(CourseCoreDbContext dbContext)
    {
        _dbContext = dbContext;
    }

    public async Task<PasswordResetToken?> FindByTokenHashAsync(
        string tokenHash,
        CancellationToken cancellationToken = default)
    {
        var model = await _dbContext.PasswordResetTokens
            .AsNoTracking()
            .FirstOrDefaultAsync(x => x.TokenHash == tokenHash, cancellationToken);

        return model is null ? null : PasswordResetTokenMapper.ToDomain(model);
    }
